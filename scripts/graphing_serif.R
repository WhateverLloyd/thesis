...
tele <- read.csv("./data/Climate_Teleconnect_1999-2018.csv")
drought <- read.csv("./data/NCDC_MonthlyDrought_SC.csv")
climate_summary <- read.csv("./data/climate_summary.csv")
...
#separate scecap data by habitat type
...
#edit county population and landcover data (remove coastal totals, create duplicate county column)
county_population           <- county_population %>% filter(COUNTY != "SC_COASTAL")
county_population$COUNTY2   <- county_population$COUNTY

landcover_counties         <- landcover_counties %>% filter(COUNTY != "SC_COASTAL")
landcover_counties$COUNTY2 <- landcover_counties$COUNTY

#convert landcover data from wide to long format
landcover_long <- landcover_counties %>% 
  select(COUNTY, YEAR, Developed, Agriculture, 
         Forested, Marsh, Water) %>%
  gather(key = "LandcoverCategory", value = "Percent", 
         Developed, Agriculture, Forested, Marsh, Water)

#order landcover categories in the gain/lost dataset as factors
...
#prepare teleconnection data
#convert date and convert from wide to long format

tele$DATE <- as.Date(tele$START_DATE, format = "%m/%d/%Y" )
tele <- tele %>% filter(YEAR %in% c(1999:2018)) %>%
  select(DATE, AMO, AO, NAO, ONI) %>% 
  gather(., key = "Index", value = "Value", -DATE)
tele$Index2 <- tele$Index

#prepare drought data
#convert date, separate into climate divisions

drought$DATE <- as.Date(drought$START_DATE, format = "%m/%d/%Y")
drought$SC_CLIM_DIV <- as.character(drought$SC_CLIM_DIV)
droughtdiv4  <- filter(drought, SC_CLIM_DIV == "4")
droughtdiv7  <- filter(drought, SC_CLIM_DIV == "7")

#split into palmer based drought indices and standard precipitation indices
#and convert from wide to long format

drought_PALMER4 <- droughtdiv4 %>% filter(YEAR %in% c(1999:2018)) %>%
  select(DATE, PDSI, PHDI, PMDI, ZNDX) %>%
  gather(., key = "Index", value = "Value", -DATE)
drought_PALMER4$Index2 <- drought_PALMER4$Index

drought_PALMER7 <- droughtdiv7 %>% filter(YEAR %in% c(1999:2018)) %>%
  select(DATE, PDSI, PHDI, PMDI, ZNDX) %>%
  gather(., key = "Index", value = "Value", -DATE)
drought_PALMER7$Index2 <- drought_PALMER7$Index

drought_SP4 <- droughtdiv4 %>% filter(YEAR %in% c(1999:2018)) %>%
  select(DATE, SP01, SP02, SP03, SP06, SP09, SP12, SP24) %>%
  gather(., key = "Index", value = "Value", -DATE)
drought_SP4$Index2 <- drought_SP4$Index

drought_SP7 <- droughtdiv4 %>% filter(YEAR %in% c(1999:2018)) %>%
  select(DATE, SP01, SP02, SP03, SP06, SP09, SP12, SP24) %>%
  gather(., key = "Index", value = "Value", -DATE)
drought_SP7$Index2 <- drought_SP7$Index

#split climate data into annual and season, refactor seasons

climate_annual <- climate_summary %>% filter(SEASON == "Annual")
climate_season <- climate_summary %>% filter(SEASON != "Annual")
climate_season$SEASON <- factor(climate_season$SEASON, levels = c("Winter", "Spring","Summer","Fall"))
...
seasonal_temperature_faceted <- ggarrange(facet_djf_tavg,facet_mam_tavg,
                                          ...
                                          nrow = 2, ncol =2, align = "hv") %>%
  annotate_figure(top = text_grob("Seasonal Temperature Data by Weather Station (1999-2018)", 
                                  family = "Times New Roman", face = "bold", size = 14))
...
seasonal_precip_faceted <- ggarrange(facet_djf_prcp, facet_mam_prcp,
                                     ...
                                     nrow = 2, ncol =2, align = "hv") %>%
  annotate_figure(top = text_grob("Seasonal Precipitation Data by Weather Station (1999-2018)", 
                                  family = "Times New Roman", face = "bold", size = 14))
...
annual_climate_faceted <- ggarrange(facet_ann_tavg, facet_ann_prcp,
  ...
  annotate_figure(top = text_grob("Annual Climate Data by Weather Station (1999-2018)", 
                                  family = "Times New Roman", face = "bold", size = 14))
...
extreme_climate_faceted <- ggarrange(facet_extremetemp, facet_extremeprcp,
                                     nrow = 1, ncol = 2, align = "hv") %>%
  annotate_figure(top = text_grob("Annual Extreme Temperature and Precipitation Events by Weather Station (1999-2018)",
                                  family = "Times New Roman", face = "bold", size = 14))
...
wqplots2 <- ggarrange(wqplot_TEMP2, wqplot_DO2, wqplot_SAL2, 
  ...
  annotate_figure(top = text_grob("SCECAP Water Quality Data (1999 to 2018)", 
                                  family = "Times New Roman", face = "bold", size = 14))
...
facet_waterquality <- ggarrange(TEMP_facet, DO_facet,
                                ...
                                nrow = 3, ncol = 2, align = "hv") %>%
  annotate_figure(top = text_grob("SCECAP Water Quality Data (1999-2018)", 
                                  family = "Times New Roman", face = "bold", size = 14))
...
facet_sediment <- ggarrange(MET_facet, ERMQMET_facet,
  ...
  annotate_figure(top = text_grob("SCECAP Sediment Quality Data (1999-2018)", 
                                  family = "Times New Roman", face = "bold", size = 14))
...
facet_biological <- ggarrange(trawlrich_facet, trawlabund_facet,
                              ...
                              nrow = 3, ncol = 2, align = "hv") %>%
                    annotate_figure(top = text_grob("SCECAP Biological Quality Data (1999-2018)",
                                                    family = "Times New Roman", face = "bold", size = 14))
...
ggsave("./outputs/timeseries_faceted_biological.png",
...
waterquality_boxplots_habitats <- ggarrange(boxplot_habitat_temp, boxplot_habitat_DO,
  ...
  annotate_figure(top = text_grob("SCECAP Water Quality Data by Habitat Type", 
                                  family = "Times New Roman", face = "bold", size = 14))
...
logsedimentquality_boxplots_habitats <- ggarrange(boxplot_habitat_logmet, boxplot_habitat_logmetermq,
                                                  ...
                                                  nrow = 5, ncol = 2, align = "hv") %>%
  annotate_figure(top = text_grob("SCECAP Sediment Quality Data by Habitat Type", 
                                  family = "Times New Roman", face = "bold", size = 14))
...
boxplot_habitats_biological <- ggarrange(boxplot_habitat_trawlrich, boxplot_habitat_trawlabund,
                                         ...
                                         nrow = 3, ncol = 2, align = "hv") %>% 
  annotate_figure(top = text_grob("SCECAP Biological Quality Data by Habitat Type",
                                  family = "Times New Roman" , face = "bold", size = 14))
...
waterquality_boxplots_periods <- ggarrange(boxplot_period_temp , boxplot_period_DO,
                                           ...
                                           ncol = 2, nrow = 3, align = "hv") %>%
  annotate_figure(top = text_grob("SCECAP Water Quality Data by Time Period", 
                                  family = "Times New Roman", face = "bold", size = 14))
...
facetwaterquality_boxplots_periods <- ggarrange(facetboxplot_period_temp , facetboxplot_period_DO,
                                                ...
                                                ncol = 2, nrow = 3, align = "hv") %>%
  annotate_figure(top = text_grob("SCECAP Water Quality Data by Time Period and Habitat Type",
                                  family = "Times New Roman", face = "bold", size = 14))
...
logsedimentquality_boxplots_periods <- ggarrange(boxplot_period_logmet, boxplot_period_logmetermq,
                                                 ...
                                                 nrow = 5, ncol = 2, align = "hv") %>%
  annotate_figure(top = text_grob("SCECAP Sediment Quality Data by Time Period", 
                                  family = "Times New Roman", face = "bold", size = 14))
...
facetlogsedimentquality_boxplots_periods <- ggarrange(facetboxplot_period_logmet, facetboxplot_period_logmetermq,
                                                      ...
                                                      nrow = 5, ncol = 2, align = "hv") %>%
                                            annotate_figure(top = text_grob("SCECAP Sediment Quality Data by Time Period and Habitat Type",
                                                            family = "Times New Roman", face = "bold", size = 14))
...
boxplot_periods_biological <- ggarrange(boxplot_period_trawlrich, boxplot_period_trawlabund,
                                        ...
                                        nrow = 3, ncol = 2, align = "hv") %>% 
                              annotate_figure(top = text_grob("SCECAP Biological Quality Data by Time Period", 
                                                              family = "Times New Roman", face = "bold", size = 14))
...
facetboxplot_periods_biological <- ggarrange(facetboxplot_period_trawlrich, facetboxplot_period_trawlabund,
                                             ...
                                             nrow = 3,  ncol = 2, align = "hv") %>% 
                                  annotate_figure(top = text_grob("SCECAP Biological Quality Data by Time Period and Habitat Type", 
                                                                  family = "Times New Roman", face = "bold", size = 14))
...
waterquality_boxplots_developments <- ggarrange(boxplot_development_temp, boxplot_development_DO,
                                                ...
                                                ncol = 2, nrow = 3, align = "hv") %>%
                                      annotate_figure(top = text_grob("SCECAP Water Quality Data by Watershed Development", 
                                                                      family = "Times New Roman", face = "bold", size = 14))
...
facetwaterquality_boxplots_developments <- ggarrange(facetboxplot_development_temp, facetboxplot_development_DO,
                                                     ...
                                                     ncol = 2, nrow = 3, align = "hv") %>%
                                          annotate_figure(top = text_grob("SCECAP Water Quality Data by Watershed Development and Habitat Type", 
                                                                          family = "Times New Roman", face = "bold", size = 14))
...
logsedimentquality_boxplots_developments <- ggarrange(boxplot_development_logmet, boxplot_development_logmetermq,
                                                      ...
                                                      nrow = 5, ncol = 2, align = "hv") %>%
                                            annotate_figure(top = text_grob("SCECAP Sediment Quality Data by Watershed Development",
                                                                            family = "Times New Roman", face = "bold", size = 14))
...
facetlogsedimentquality_boxplots_developments <- ggarrange(facetboxplot_development_logmet, facetboxplot_development_logmetermq,
                                                           ...
                                                           nrow = 5, ncol = 2, align = "hv") %>%
                                                annotate_figure(top = text_grob("SCECAP Sediment Quality Data by Watershed Development and Habitat Type",
                                                                                family = "Times New Roman", face = "bold", size = 14))
...
boxplot_developments_biological <- ggarrange(boxplot_development_trawlrich, boxplot_development_trawlabund,
                                             ...
                                             ncol = 2, align = "hv") %>% 
                                    annotate_figure(top = text_grob("SCECAP Biological Quality Data by Watershed Development", 
                                                                    family = "Times New Roman", face = "bold", size = 14))
...
facetboxplot_developments_biological <- ggarrange(facetboxplot_development_trawlrich, facetboxplot_development_trawlabund,
                                                  ...
                                                  nrow = 3, ncol = 2, align = "hv") %>% 
                                        annotate_figure(top = text_grob("SCECAP Biological Quality Data by Watershed Development and Habitat Type",
                                                                        family = "Times New Roman", face = "bold", size = 14))
...
impervious_by_county <- landcover_counties %>%
                        ggplot( aes(x=YEAR, y=MEAN_UPLAND_IMPERV_PRCT)) +
                        geom_line(data=landcover_counties %>% dplyr::select(-COUNTY),
                                  aes(group=COUNTY2), color = "black", linetype=1, size=0.5, alpha = 0.3) +
                        geom_line(color="black", size=1)+
...
landcover_by_county <-  ggplot(data = landcover_long, aes(x= YEAR, y = Percent, group = LandcoverCategory)) +
                        geom_point(aes(shape = LandcoverCategory)) + 
                        scale_shape_manual(values = c(0,17,16,18,1)) +
                        labs(x = "Year", y = "Percent") +
                        geom_line(size = 0.3, color = "black", alpha = 1) + 
                        theme_classic(base_family = "Times New Roman") + 
                        facet_wrap(~COUNTY) 

landcover_by_county <-  landcover_by_county %>% 
                        annotate_figure(top = text_grob("Changes in Landcover in Coastal South Carolina Counties (2001-2016)", 
                                              family = "Times New Roman", face = "bold", size = 14))
...
coastal_landchange <- ggplot(data = gain_lost, aes(x= Landcover, y = Hectares, fill = Change)) +
                      ...
                      theme(legend.title = element_blank(), title = element_text(hjust = 0.5, face = "bold"),
...
#4. PLOTTING CLIMATE TELECONNECTIONS AND DROUGHT DATA----
#4.1. Climate Teleconnection----
teleconnections_graph <-  ggplot(data = tele, aes(x=DATE, y=Value)) +
                          geom_line(color="black", size=0.5) +
                          theme_classic(base_family = "Times New Roman") +
                          labs(x = "Year", y = "Index Value") +
                          facet_wrap(~Index)

teleconnections_graph <-  teleconnections_graph %>%
                          annotate_figure(top = text_grob("Climate Teleconnections (1999-2018)",
                                          face = "bold", size = 14, family = "Times New Roman"))

#4.2. Drought Data----
#4.2.1. Standard Precipitation Index----

drought_SPI_4 <-  drought_SP4 %>% filter(Index != "SP02") %>%
                  ggplot(aes(x=DATE, y=Value)) +
                  geom_line(data=drought_SP4 %>% dplyr::select(-Index),
                            aes(group=Index2), color = "black", linetype=1, size=0.3, alpha = 0.3) +
                  geom_line(color="black", size=0.5)+
                  theme_classic(base_family = "Times New Roman") +
                  labs(x = "", y = "Index Value", title = "SC Climate Division 4") +
                  facet_wrap(~Index) 

drought_SPI_7 <-  drought_SP7 %>% filter(Index != "SP02") %>%
                  ggplot(aes(x=DATE, y=Value)) +
                  geom_line(data=drought_SP7 %>% dplyr::select(-Index),
                            aes(group=Index2), color = "black", linetype=1, size=0.3, alpha = 0.3) +
                  geom_line(color="black", size=0.5)+
                  theme_classic(base_family = "Times New Roman") +
                  labs(x = "Year", y = "Index Value", title = "SC Climate Division 7") +
                  facet_wrap(~Index) 

SPI_drought_graphs  <-  ggarrange(drought_SPI_4, drought_SPI_7, 
                                  nrow = 2, ncol = 1, align = "hv") %>%
                        annotate_figure(top = text_grob("Standard Precipitation Indices for Coastal South Carolina (1999-2018)",
                                        face = "bold", size = 14, family = "Times New Roman"))

#4.2.2. Drought Data----

drought_PALMER4graph <- drought_PALMER4 %>%
                        ggplot(aes(x=DATE, y=Value)) +
                        geom_line(data=drought_PALMER4 %>% dplyr::select(-Index),
                                  aes(group=Index2), color = "black", linetype=1, size=0.3, alpha = 0.3) +
                        geom_line(color="black", size=0.5)+
                        theme_classic(base_family = "Times New Roman") +
                        labs(x = "", y = "Index Value", title = "SC Climate Division 4") +
...
drought_PALMER7graph <- drought_PALMER7 %>% 
                        ggplot(aes(x=DATE, y=Value)) +
                        geom_line(data=drought_PALMER7 %>% dplyr::select(-Index),
                                  aes(group=Index2), color = "black", linetype=1, size=0.3, alpha = 0.3) +
                        geom_line(color="black", size=0.5)+
                        theme_classic(base_family = "Times New Roman") +
                        labs(x = "Year", y = "Index Value", title = "SC Climate Division 7") +
...
palmer_drought_graphs <-  ggarrange(drought_PALMER4graph, drought_PALMER7graph, 
                                    nrow = 2, ncol=1, align = "hv") %>%
                          annotate_figure(top = text_grob("Drought Index Values for Coastal South Carolina (1999-2018)", 
                                          face = "bold", size = 14, family = "Times New Roman"))
#4.3. Climate Plots

climate_plot_annual <-    ggplot(data = climate_annual, aes(x = PRCP, y = TAVG, label = YEAR)) + 
                          annotate("text", x = c(-25, -25, 25, 25), y = c(0.5, -0.5, 0.5, -0.55), 
                                   label = c("Warm & Dry","Cool & Dry","Warm & Wet","Cool & Wet"), 
                          ...
                          geom_text_repel(force = 1, family = "Times New Roman") +
                          labs(x = "Total Precipitation Difference from Normal (cm)", 
                               y = "Average Temperature Difference From Normal (°C)", 
                               title = "Annual Climate for Coastal South Carolina (1999-2018)") +
                          theme_classic(base_family = "Times New Roman") +
                          theme(plot.title = element_text(hjust = 0.5, face = "bold")) 

climate_plot_seasonal <-  ggplot(data = climate_season, aes(x = PRCP, y = TAVG, label = YEAR)) + 
                          geom_point() +
                          geom_hline(yintercept = 0, linetype = 3) +
                          geom_vline(xintercept = 0, linetype = 3) +
                          ...
                          labs(x = "Total Precipitation Difference from Normal (cm)", 
                               y = "Average Temperature Difference From Normal (°C)", 
                               title = "Seasonal Climate for Coastal South Carolina (1999-2018)") +
                          theme_classic(base_family = "Times New Roman") +
                          theme(plot.title = element_text(hjust = 0.5, face = "bold")) +
                          facet_wrap(~SEASON)
...
ggsave("./outputs/climate_teleconnect.png",
       teleconnections_graph, width = 250, height = 200, units = "mm",dpi = "retina")

ggsave("./outputs/drought_SPI.png",
       SPI_drought_graphs, width = 250, height = 200, units = "mm",dpi = "retina")

ggsave("./outputs/drought_palmer.png",
       palmer_drought_graphs, width = 250, height = 250, units = "mm",dpi = "retina")

ggsave("./outputs/climate_plot_annual.png",
       climate_plot_annual, width = 200, height = 200, units = "mm",dpi = "retina")

ggsave("./outputs/climate_plot_seasonal.png",
       climate_plot_seasonal, width = 200, height = 200, units = "mm",dpi = "retina")
